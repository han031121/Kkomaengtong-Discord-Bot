...
import { formatDateInTimeZone, getPreviousWordlePrintDate } from "../domain/print-date.js";
import type { WordlePuzzleClient } from "./ports.js";
...
export class WordlePuzzleCache {
    ...
    public constructor(
        private readonly client: WordlePuzzleClient,
    ...
    public async stopRefreshes(): Promise<void> {
        ...
        await Promise.all(this.activeRefreshes);
    }

    public async stopDailyRefresh(): Promise<void> {
        await this.stopRefreshes();
    }
    ...
    private scheduleRefreshAttempt(
        ...
        const refreshTimer = setTimeout(() => {
            ...
            const refreshTask = this.refreshDuringWindow(
                windowStartTimeMs,
                attemptIndex,
                refreshWindowKind,
            )
                .catch((error: unknown) => {
                    console.error("Wordle 예약 갱신 작업을 완료하지 못했습니다.", error);
                })
                .finally(() => {
                    this.activeRefreshes.delete(refreshTask);
                });

            this.activeRefreshes.add(refreshTask);
        }, delayMs);
